/**
 * Utility module to generate public URLs for verification links
...
module.exports = {
  ...
  generatePublicUrl: (req, token) => {
    ...
    if (publicBaseUrl) {
      return `${publicBaseUrl}/service/verify/verifyAndRedirect?token=${token}&redirect=${redirectParam}`;
    }
    ...
    return `${finalProtocol}://${host}/service/verify/verifyAndRedirect?token=${token}&redirect=${redirectParam}`;
  }
};
